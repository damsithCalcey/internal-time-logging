// Barrel: use cases import typed repo objects from here.
// Each repository file owns its own contract (the *Repo interface) and asserts
// the binding via `satisfies`. Use cases depend on the binding object, not the
// individual named exports — `import { usersRepo } from '@/db/repositories'`.
//
// The "inline Drizzle in a use case" drift is structurally prevented by:
//   1. This boundary (use cases touch the bound API, not the schema), and
//   2. The `no-drizzle-in-use-cases` ESLint rule (apps/api/eslint-local.mjs)
//      that fails CI if a use case or route file imports drizzle-orm or db/schema.
