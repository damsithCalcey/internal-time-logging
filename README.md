...
- **Monoliths first, modularity later.** Every page was initially built as one large component. The last ~10 commits on `main` are almost entirely "Refactor — break apart the X monolith" (Projects, Team, Daily Log, Weekly Summary, Approvals) and adjacent extractions: a shared cache-invalidation helper (`e6a296c`), a `StatusBadge` component (`080cdd7`), dedicated serializers (`6714148`, `9a8aa3f`), and splitting `updateTimeEntry` into employee vs manager paths (`c532087`). The architecture was sound; the first pass per slice wasn't.
